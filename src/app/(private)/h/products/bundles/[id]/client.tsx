...
export default function BundleDetailsClient({ id }: { id: string }) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const isEditMode = searchParams.get("edit") === "true";
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);

  const { data: bundle, isLoading: isBundleLoading } = useBundle(id);
  const { data: products, isLoading: isProductLoading } =
    useProductsByBundleId(id);
  ...
  const handleSubmit = (data: BundleFormValues) => {
    ...
    router.push(`/h/products/bundles/${id}`);
  };
      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            ...
            <Button
              onClick={() => router.push(`/h/products/bundles/${id}?edit=true`)}
            >
              <IconEdit />
              Edit Bundle
            </Button>
